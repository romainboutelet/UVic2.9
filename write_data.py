...
lon1=19*102*102
R=0.5
covR=np.array([3e-1,5e-5,6e-3,6e-5,4e-15])*R
file_name=[]
newfile=[]
for i in range(20):
	yr=str(44903+i)
	file_name.append(Dataset('../'+dirname+'/'+dataname+'/rest.'+yr+'.01.01.nc' , 'r+', format='NETCDF4'))	
	newfile.append(Dataset('../'+dirname+'/data05_lgm20yr/rest.'+yr+'.01.01.nc' , 'w', format='NETCDF4'))
...
#Copy variables
	...
	for var in iter(file_name[i].variables):
		...
		if j0>=len(lvar0) and j1>=len(lvar1):
			...
			outvar[:]=(file_name[i][lvar1[j1]][:]!=0)*(file_name[i][lvar1[j1]][:]+np.fromfunction(lambda i,j,k,l:np.random.normal(0*i*j*k*l,noise(var,lvar0,lvar1,covR)),file_name[i][lvar1[j1]][:].shape))
			j1=j1+1
			...
			outvar.setncatts({k: file_name[i].variables[var].getncattr(k) for k in file_name[i].variables[var].ncattrs()})
			outvar[:]=(file_name[i][lvar0[j0]][:]!=0)*(file_name[i][lvar0[j0]][:]+np.fromfunction(lambda i,j,k,l:np.random.normal(0*i*j*k*l,noise(var,lvar0,lvar1,covR)),file_name[i][lvar0[j0]][:].shape))
			j0=j0+1
